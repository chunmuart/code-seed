...
if __name__ == "__main__":
    ...
    os.environ["PYTORCH_CUDA_ALLOC_CONF"] = "max_split_size_mb:128"  # 增加，以避免小块碎片无法重用：
    # ---------------------------------#
    #   Cuda    是否使用Cuda
    #           没有GPU可以设置成False
    # ---------------------------------#
    ...
    Init_Epoch = 0
    Freeze_Epoch = 50
    Freeze_batch_size = 4  # 可减小一些，4？原8
    # Freeze_lr = 5e-4    #可去？
    # ------------------------------------------------------------------#
    #   解冻阶段训练参数
    ...
    if local_rank == 0:
        ...
        # ----------------------------------------------------------#
        wanted_step = 1001 if optimizer_type == "sgd" else 0.5e4
